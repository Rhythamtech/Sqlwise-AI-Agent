...
def ingest_order_items(csv_path):
    """
    Ingests order_items from CSV into MSSQL table with USD to INR conversion using BCP for maximum speed.
    """
    ...
    insert_cols = [
        'order_item_id', 'order_id', 'created_at', 'product_id', 'is_primary_item', 
        'price_usd', 'price_inr', 'cogs_usd', 'cogs_inr'
    ]
    ...
    temp_csv = "temp_order_items_bcp.csv"
    print(f"Saving temporary data for BCP...")
    df_clean.to_csv(temp_csv, sep='|', index=False, header=False)

    # Run BCP
    try:
        success = run_bcp('order_items', temp_csv, separator='|')
        if success:
            print("Order Items ingestion completed via BCP.")
    finally:
...
if __name__ == "__main__":
    ...
    csv_file = os.path.join(base_dir, 'files', 'order_items.csv')
    ingest_order_items(csv_file)
